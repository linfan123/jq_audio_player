...
;(function($){
    var rotatetimer,    /* 旋转定时器 */
        ...
        $btnFf = $('.btn_ff'), //快进
        $btnFr = $('.btn_fr'), //快退
        $timeCur = $('#time_cur'), //当前播放时间
        $btnNext = $('.btn_next'), //下一首
        $btnPrev = $('.btn_prev'),  //上一首
        $audioDot = $('.audio_dot'), //进度条小滑块
        $audioBlue = $('.audio_blue'), //进度条已播放的蓝色区域
        $audioLine = $('.audio_line'), //整个进度条灰色部分
        $audioTitle = $('.audio_title'), //歌曲名
        $lyricList = $("#lyric_list"), //歌词列表
        $singer = $('#singer'), //歌手信息
        $songList = $('.song_list'), //歌曲列表
        wLine = $audioLine.width(), //整个进度条灰色部分宽度
        wDot = $audioDot.width(), //进度条小滑块的宽度
        $timeLong = $('#time_long'), //歌曲总时长
        $music=$('#music'), //音乐播放器
   ...
   function renderLyric(i){
    ...
    function updateLyric() {
        ...
        $li.addClass('on').siblings().removeClass('on');  /* 高亮 */
        nTop = $li.attr('data-line');                       /* 获取 当前行歌词 的行数 */

        if(nTop >4){                                        /*歌词高亮显示在第四行*/
            $lyricList.stop().animate({top:-(nTop-4)*lineHeight}); /* 开始滚动 */
        }
    }
})(jQuery);